...
public class FreerunPlayer extends PlayerBase
{
	...
	public float getSpeedModifier()
	{
		...
		if (!player.isInWater() && !player.handleLavaMovement())
		{
			if (player.onGround && !isClimbing)
			{
				...
				} /*else if (FreeRun.barWood.blockID!=0 && isOnCertainBlock(FreeRun.barWood.blockID))
				{FIXME
					return 0.5F;
				}*/
			}
	...
	private float roll(float f)
	{
		...
		int k = MathHelper.floor_double(player.posZ);
		int b = player.worldObj.getBlockId(i, j, k);
		if (b == Block.fence.blockID || b == Block.netherFence.blockID || isOnCertainBlock(Block.leaves.blockID) || isOnCertainBlock(FreeRun.barWood.blockID) || player.isInWater())
		{
			f /= 2F;
			return f;
		}
		...
		if (objectMouseOver != null && objectMouseOver.typeOfHit == EnumMovingObjectType.TILE)
		{
			if (isSelectedBlockClose(2.0F) && isSelectedBlockOnLevel(0))
			{
				...
				if (m.isSolid() && isBlockAboveAir(2, false, isClimbing))
				{
					...
					{
						return 2;
					} else if (Block.blocksList[b].getBlockBoundsMaxY() > player.stepHeight && Block.blocksList[b].getBlockBoundsMaxY() <= 1.0F && b != Block.stairCompactPlanks.blockID && b != Block.stairCompactCobblestone.blockID)
					{
						return 1;
					}
				}
	...
	public void animateRoll(float f)
	{
		if (ModLoader.getMinecraftInstance().gameSettings.thirdPersonView == 0)
		{
			...
			player.realSetRotation(yaw, pitch);
		}
	}
